...
/** textはカスタムする場合はlabel同様にoverride して利用してください。 textの用途はAPIで数字でやりとりをせずにtextでやる(※他社のAPIなども参考にしてます)事を目的としています。
  * プログラム内部で使う事を想定していません。
  *
  * labelは数字を表示する際にわざわざ利用側で変換テーブルを持たなくていいように設計してる。 また日本語の意味がわかりやすいのもあり実装してる。
  *
